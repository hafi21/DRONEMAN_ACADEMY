...
export default function Courses() {
  ...
  return (
    <section id="courses" className="py-24 relative overflow-hidden bg-gradient-to-b from-card/30 to-background">
      ...
      <motion.div
        ...
        style={{
          backgroundImage: 'radial-gradient(circle at 50% 50%, rgba(123, 97, 255, 0.1) 0%, transparent 50%)',
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-sm font-bold tracking-widest text-secondary uppercase mb-4">Learn to Fly</h2>
          <h3 className="text-4xl md:text-5xl font-extrabold mb-6">
            Elite <span className="text-gradient">Drone Courses</span>
          </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {courses.map((course, index) => (
            <motion.div
              ...
              className={`relative glass-panel rounded-3xl p-8 border transition-all duration-300 hover:-translate-y-2 ${
                course.popular ? 'border-primary shadow-[0_0_30px_rgba(123,97,255,0.2)]' : 'border-border/50 hover:border-primary/50'
              }`}
            >
              {course.popular && (
              ...
              <div className="flex justify-center gap-4 mb-8">
                <div className="flex items-center gap-1 text-xs font-medium bg-background/50 px-3 py-1.5 rounded-full border border-border/50">
                  <Clock className="w-4 h-4 text-primary" /> {course.duration}
                </div>
                <div className="flex items-center gap-1 text-xs font-medium bg-background/50 px-3 py-1.5 rounded-full border border-border/50">
                  <Users className="w-4 h-4 text-secondary" /> {course.mode}
                </div>
              </div>
                ...
                <a
                  ...
                  className={`inline-block w-full py-4 rounded-xl font-bold transition-all ${
                    course.popular
                      ...
                      : 'bg-white/5 text-foreground border border-border/50 hover:bg-white/10'
                  }`}
                >
                  Enroll Now
                </a>
